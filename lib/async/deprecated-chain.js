  ...
  if (!_this._queue) {
    _this._queue = async.queue(function (task, callback) {
      if (task.args.length > 0 &&
        typeof task.args[task.args.length - 1] === 'function') {
        ...
        var cbArg = (task.name === 'queueAddAsync' ? 1 : task.args.length - 1);
        var func = task.args[cbArg];
        task.args[cbArg] = function (err) {
