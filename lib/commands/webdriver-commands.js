var fs = require('fs'),
    ...
    codeToString = helpers.codeToString,
 ...
 * @docOrder 1
 */
commands.execute = function (code, args) {
...
var safeExecuteJsScript =
  helpers.inlineJs(fs.readFileSync(
    __dirname + '/../../browser-scripts/safe-execute.js', 'utf8'));
 ...
 * @docOrder 2
 */
commands.safeExecute = function (code, args) {
  code = codeToString(code);
  args = args || [];
  return this._jsonWireCall({
    method: 'POST',
 ...
 */
commands.executeAsync = function (code, args) {
  code = codeToString(code);
  args = args || [];
  return this._jsonWireCall({
    method: 'POST',
...
var safeExecuteAsyncJsScript =
  helpers.inlineJs(fs.readFileSync(
    __dirname + '/../../browser-scripts/safe-execute-async.js', 'utf8'));
 ...
 * @jsonWire POST /session/:sessionId/execute_async
 */
commands.safeExecuteAsync = function (code, args) {
  code = codeToString(code);
  args = args || [];
  return this._jsonWireCall({
    method: 'POST',
 ...
 */
commands.newWindow = function (url, name) {
 ...
 */
commands.getWindowSize = function (win) {
  win = win || 'current';
  return this._jsonWireCall({
    method: 'GET',
    relPath: '/window/' + win + '/size'
...
/**
  ...
  * @jsonWire POST /session/:sessionId/window/:windowHandle/size
 */
commands.setWindowSize = function (width, height, win) {
  win = win || 'current';
  return this._jsonWireCall({
    method: 'POST',
    relPath: '/window/' + win + '/size',
 ...
 */
commands.getWindowPosition = function (win) {
  win = win || 'current';
  return this._jsonWireCall({
    method: 'GET',
 ...
 */
commands.setWindowPosition = function (x, y, win) {
  win = win || 'current';
  return this._jsonWireCall({
    method: 'POST',
    relPath: '/window/' + win + '/position',
...
commands.setHttpTimeout = function (ms) {
  deprecator.warn('setHttpTimeout',
    'setHttpTimeout/setHTTPInactivityTimeout has been deprecated,' +
    ' use configureHttp instead.');
 ...
 *
 */
commands.configureHttp = function (opts) {
 ...
 * (use setImplicitWaitTimeout and setAsyncScriptTimeout to set the other 
 * timeouts)
 *
...
/**
 * Retrieve an element avoiding not found exception and returning undefined 
 * instead:
...
commands.waitFor = function () {
  ...
  var opts;
  // retrieving options
  if (typeof args[0] === 'object' && !(args[0] instanceof Asserter)) {
    opts = args[0];
  } else
  {
    opts = {
  ...
  var _assert;
  if (opts.asserter instanceof AsyncAsserter) {
    _assert = function () {
      return Q.nfcall(opts.asserter.assert, _this._asyncFacade)
        .then(function (res) {
        var satisfied = _.isArray(res) ? res[0] : res;
        if (satisfied) {
          return res[1]; // value
        } else {
    ...
    _assert = opts.asserter.assert.bind(null,  _this);
  }
...
commands.waitForElement = function () {
  ...
  // retrieving options
  if (typeof args[2] === 'object' && !(args[2] instanceof Asserter)) {
    opts = args[2];
  } else if (args[2] instanceof Asserter) {
    opts = {
      asserter: args[2],
      timeout: args[3],
      pollFreq: args[4]
  ...
  // default
  opts.asserter = opts.asserter || new Asserter(function (el) {
    return new Q(el);
  });
  ...
  var wrappedAsserter = new Asserter(
    function (browser) {
      var _assert;
      if (opts.asserter instanceof AsyncAsserter) {
        var AsyncElement = require('../async/element-async');
        _assert = function (el) {
          var asyncEl = new AsyncElement(el.value, browser);
          return Q.nfcall(opts.asserter.assert, asyncEl).then(function (res) {
            var satisfied = _.isArray(res) ? res[0] : res;
            if (satisfied) {
              return res[1]; // value
            } else {
        ...
        _assert = opts.asserter.assert;
      }

      return commands.elements.apply(browser, [using, value])
        .then(function (els) {
          if (els.length > 0) {
            return _assert(els[0]).thenResolve(els[0]);
          }
          else {
            var err = new Error('Condition not satisfied');
            err.retriable = true;
...
commands.waitForVisible = function (using, value, timeout, pollFreq) {
  deprecator.warn('waitForVisible',
    'waitForVisible has been deprecated, use waitForElement + ' +
    'isVisible asserter instead.');
  return commands.waitForElement
    .apply(this, [using, value, predefinedAsserters.isVisible,
      timeout, pollFreq])
    .catch(function (err) {
      if (err && err.message &&
          err.message.match(/Element condition wasn\'t satisfied!/)) {
 ...
 * the screenshot name, or left blank (will create a file in the system 
 * temp dir).
 */
commands.saveScreenshot = function (_path) {
  var _this = this;
...
var addMethodsForSuffix = function (type, singular, plural) {
  if (singular) {
     ...
     * @jsonWire POST /session/:sessionId/element
     */
    commands['element' + helpers.elFuncSuffix(type)] = function () {
      var args = helpers.varargs(arguments).array;
      args.unshift(helpers.elFuncFullType(type));
     ...
     * @docOrder 4
     */
    commands['element' + helpers.elFuncSuffix(type) + 'OrNull'] = function () {
      var args = helpers.varargs(arguments).array;
      args.unshift(helpers.elFuncFullType(type));
     ...
     * @docOrder 6
     */
    commands['element' + helpers.elFuncSuffix(type) + 'IfExists'] =
      function () {
        var args = helpers.varargs(arguments).array;
        args.unshift(helpers.elFuncFullType(type));
     ...
     * @docOrder 8
     */
    commands['hasElement' + helpers.elFuncSuffix(type)] = function () {
      var args = helpers.varargs(arguments).array;
      args.unshift(helpers.elFuncFullType(type));
     ...
     */
    /* jshint ignore:end */
    commands['waitForElement' + helpers.elFuncSuffix(type)] = function () {
      var args = helpers.varargs(arguments).array;
      args.unshift(helpers.elFuncFullType(type));
      return commands.waitForElement.apply(this, args);
    };

    commands['waitForVisible' + helpers.elFuncSuffix(type)] = function () {
      var args = helpers.varargs(arguments).array;
      args.unshift(helpers.elFuncFullType(type));
  ...
  if (plural) {
    commands['elements' + helpers.elFuncSuffix(type)] = function () {
      var args = helpers.varargs(arguments).array;
      args.unshift(helpers.elFuncFullType(type));
...
_.each(helpers.elementFuncTypes, function (suffix) {
 ...
 * @docOrder 1
 */
commands.getAttribute = function (element, attrName) {
 ...
 * @docOrder 3
 */
commands.getValue = function (element) {
  if (!element) { throw new Error('Missing element.'); }
...
var _flick1 = function (xspeed, yspeed, swipe) {
...
var _flick2 = function (element, xoffset, yoffset, speed) {
  return this._jsonWireCall({
    method: 'POST',
    relPath: '/touch/flick',
 ...
 */
commands.moveTo = function (element, xoffset, yoffset) {
  return this._jsonWireCall({
    method: 'POST',
    relPath: '/moveto',
    data: { element:
 ...
 */
commands.buttonDown = function (button) {
  return this._jsonWireCall({
    method: 'POST',
    relPath: '/buttondown',
    data: {button: button}
 ...
 */
commands.buttonUp = function (button) {
  return this._jsonWireCall({
    method: 'POST',
    relPath: '/buttonup',
    data: {button: button}
 ...
 */
commands.click = function (button) {
  // button optional
  return this._jsonWireCall({
    method: 'POST',
    relPath: '/click',
 ...
 * @docOrder 1
 */
commands.text = function (element) {
  var _this = this;
 ...
 */
commands.setLocalStorageKey = function (key, value) {
 ...
 */
commands.getLocalStorageKey = function (key) {
 ...
 */
commands.removeLocalStorageKey = function (key) {
  return commands.safeExecute.apply(
    this,
...
commands.uploadFile = function (filepath) {
  var _this = this;
  return helpers.fileToBase64String(filepath).then(function (base64Data) {
...
commands.waitForJsCondition = function (jsConditionExpr, timeout, pollFreq) {
  deprecator.warn(
    'waitForJsCondition',
    'waitForJsCondition has been deprecated, use waitFor + ' +
    'jsCondition asserter instead.');

  return commands.waitFor.apply(this, [
    {
      asserter: predefinedAsserters.jsCondition(jsConditionExpr, true),
      timeout: timeout,
      pollFreq: pollFreq
    }
  ]).catch(function (err) {
...
var _waitForConditionInBrowserJsScript =
  helpers.inlineJs(fs.readFileSync(__dirname +
    '/../../browser-scripts/wait-for-cond-in-browser.js', 'utf8'));
 ...
 */
/* jshint ignore:end */
commands.waitForConditionInBrowser = function (conditionExpr, timeout, poll) {
  var _this = this;
  // parsing args
  ...
  // calling script
  return commands.safeExecuteAsync.apply(
    _this,
    [_waitForConditionInBrowserJsScript, [conditionExpr, timeout, poll]]
  ).then(function (res) {
      if (res !== true) { throw new Error(
        'waitForConditionInBrowser failure for: ' + conditionExpr);
      }
      return res;
    });
};
 ...
 */
commands.attach = function (sessionID) {
