    ...
    if(isLocalFile) {
      return webdriverCommands.uploadFile.apply(_this.browser, [path])
        .then(function (distantFilePath) {
          return webdriverCommands.type.apply(_this.browser, [_this, distantFilePath]);
        });
    } else {
...
commands.moveTo = function() {
  var fargs = helpers.varargs(arguments),
      xoffset = fargs.all[0],
      yoffset = fargs.all[1];
  return webdriverCommands.moveTo.apply(this.browser, [this,xoffset, yoffset]);
};
...
commands.flick = function (xoffset, yoffset, speed) {
  return webdriverCommands.flick.apply(this.browser, [this.value, xoffset, yoffset, speed]);
};
...
commands.getComputedCss = function(styleName) {
  return webdriverCommands.getComputedCss.apply(this.browser, [this, styleName]);
};
