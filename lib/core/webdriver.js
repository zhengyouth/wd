...
Webdriver.wrapAsyncCommand = function (name, fn) {
  ...
  return function () {
    ...
    this._asyncFacade[name].apply(this._asyncFacade,
...
Webdriver.addCommand = function (name, fn) {
  Webdriver.prototype[name] = function () {
    var _this = this;
    var args = helpers.varargs(arguments).array;
    this.emit('command', 'CALL', name, niceArgs(args));
    ...
    promise = fnRes.then(
      function (res) {
        _this.emit('command', 'RESPONSE', name + niceArgs(args),
          niceArgs(res));
        return res;
      }, function (err) {
        err.message = '[' + name + niceArgs(args) + '] ' + err.message;
        throw err;
      }
    );
...
Webdriver.prototype._init = function (desired) {
  ...
  return httpUtils
    ...
    .then(function (r) {
      ...
      if (_this.sessionID) {
        _this.emit('status', '\nDriving the web on session: ' +
          _this.sessionID + '\n');
        return [_this.sessionID, resData];
      } else {
        /* jshint ignore:start */
        /* TODO: restaure old behaviour
        when no cb
