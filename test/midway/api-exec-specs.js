...
describe('api-exec ' + env.ENV_DESC, function () {
  ...
  it('browser.safeExecute - args @skip-android', function () {
    ...
    return browser
      ...
      .then(function () {
        return browser
          .safeExecute('invalid-code> here', [6, 4])
            .should.be.rejectedWith(/status: 13/);
      });
  });
  ...
  it('browser.waitForCondition  @skip-android @skip-ios', function () {
    ...
    return browser
      ...
      .waitForCondition(exprCond, 2 * env.BASE_TIME_UNIT, 200)
        .should.eventually.be.ok
      .waitForCondition(exprCond, 2 * env.BASE_TIME_UNIT)
        .should.eventually.be.ok
      .waitForCondition(exprCond).should.eventually.be.ok
      .then(function () {
        return browser.waitForCondition('$wrong expr!!!')
          .should.be.rejectedWith(/status: 13/);
      });
  });
  ...
  it('browser.waitForConditionInBrowser  @skip-android @skip-ios', function () {
    ...
    return browser
      ...
      .waitForConditionInBrowser(exprCond, 2 * env.BASE_TIME_UNIT,
        0.2 * env.BASE_TIME_UNIT).should.eventually.be.ok
      .waitForConditionInBrowser(exprCond, 2 * env.BASE_TIME_UNIT)
        .should.eventually.be.ok
