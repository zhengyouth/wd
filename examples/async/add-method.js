...
wd.addAsyncMethod(
  ...
  function (selector, timeout/*, cb*/) {
    // 'wd.findCallback' is a small helper which looks for the callback in a 
    // safe way, and avoids hanging when the number of arguments passed is
    // wrong.
...
browser.init({browserName: 'chrome'}, function () {
  browser.get('http://admc.io/wd/test-pages/guinea-pig.html', function () {
    browser.title(function (err, title) {
      ...
      browser.elementByCssSelectorWhenReady('#your_comments', 500,
          function (err, el) {
        should.exist(el);
        browser.quit();
      });
